...
export const createPointController =
    async (input: Partial<z.infer<typeof inputSchema>>) : Promise<ReturnType<typeof createPointUseCase >> => { 
         ...
         const { data, error: inputParseError } = inputSchema.safeParse(input);
          ...
          console.log('create point controller')
         if (inputParseError) {
              throw new InputParseError('Invalid data', { cause: inputParseError });
         }

          return await createPointUseCase({
            tripId: data.tripId ,
            role: data.role ,
            index: data.index ,
            place: {
